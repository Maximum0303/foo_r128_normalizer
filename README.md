...
1. `foo_r128_normalizer_v1.6.1.fb2k-component`をダウンロードします。
...
v1.6.1では、英語表示に残っていた処理強度の範囲表記を修正し、
高DPIでのツールチップ幅と、画面を開いたままテーマを切り替えた際の
ツールチップ配色追従を改善しました。200～250%表示で設定画面が
作業領域を超える場合は、画面内へ収めて必要な方向へスクロールできます。
...
- 処理負荷をリアルタイム監視し、要調整相当が続くと自動安全補正
- 現在の処理状態を「正常／要調整／自動調整中／調整上限」で表示
- 安全な状態が3秒続くと補正をゆっくり解除して正常へ復帰

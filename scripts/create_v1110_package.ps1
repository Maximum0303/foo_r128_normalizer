...
try {
    ...
    $readmeEnglishPath = Join-Path $toolRoot "README_EN.txt"
    $glossaryPath = Join-Path $toolRoot "GLOSSARY.txt"
    $glossaryEnglishPath = Join-Path $toolRoot "GLOSSARY_EN.txt"
    ...
    $componentName = "foo_r128_normalizer_v1.11.0.fb2k-component"
    ...
    try {
        ...
        try {
            $fixedEntries = @(
                ...
                @($readmeEnglishPath, "README_EN.txt"),
                @($glossaryPath, "GLOSSARY.txt"),
                @($glossaryEnglishPath, "GLOSSARY_EN.txt"),
    ...
    $logLines.Add("DLLバージョン検証: 1.11.0")
    ...
    Write-Host ""
    Write-Host "v1.11.0のインストール用ファイルを作成しました。" `

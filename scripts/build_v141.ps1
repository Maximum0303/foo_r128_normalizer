...
$updateLog = Join-Path $toolRoot "v1.4.1_update_log.txt"
...
function Write-ErrorSummary {
    ...
    if ($contextLines.Count -gt 0) {
        @(
            "R128 Normalizer v1.4.1 build error summary"
            "重要行の前後2行を抽出しています。"
            ""
            $contextLines
        ) |
...
try {
    ...
    $projectBackup = "$projectPath.before_v1.4.1"
    ...
    Write-Host "v1.4.1のソース更新が完了しました。" `
    ...
    Write-UpdateLog (
        "Validated x64 / embedded version 1.4.1 / SHA256 $hash"
    )

    Write-Host ""
    Write-Host "v1.4.1のビルドに成功しました。" `

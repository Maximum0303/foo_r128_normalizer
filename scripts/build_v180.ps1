...
$updateLog = Join-Path $toolRoot "v1.8.0_update_log.txt"
...
function Write-ErrorSummary {
    ...
    if ($contextLines.Count -gt 0) {
        @(
            "R128 Normalizer v1.8.0 build error summary"
            "重要行の前後2行を抽出しています。"
            ""
            $contextLines
        ) |
...
try {
    ...
    $projectBackup = "$projectPath.before_v1.8.0"
    ...
    Write-Host "v1.8.0のソース更新が完了しました。" `
    ...
    Write-UpdateLog (
        "Validated x64 / embedded version 1.8.0 / SHA256 $hash"
    )

    Write-Host ""
    Write-Host "v1.8.0のビルドに成功しました。" `
